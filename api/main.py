from .db import DB
from .auth import get_current_user, require_root, require_owner_or_root, AuthUser
...
def signup(user: UserCreate) -> UserResponse:
    ...
    try:
        user_service.create_user(email=user.email, password=user.password, user_id=user.user_id)
        return UserResponse(user_id=user.user_id, email=user.email, is_root=user.user_id == 0)
...
def login(credentials: UserLogin) -> dict[str, Union[int, str]]:
    try:
        ...
        return {"user_id": int(user.uid), "message": "Login successful. Use ID token from Firebase SDK for requests."}
    except Exception as e:
        raise HTTPException(status_code=401, detail="Invalid credentials")

@app.get('/users/{user_id}', response_model=UserResponse)
def get_user(user_id: int, current_user: AuthUser = Depends(lambda uid: require_owner_or_root(uid))) -> UserResponse:
    user = user_service.get_user(user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return UserResponse(user_id=user['id'], email=user['email'], is_root=user['id'] == 0)

@app.put('/users/{user_id}', response_model=UserResponse)
def update_user(user_id: int, update_data: UserUpdate, current_user: AuthUser = Depends(lambda uid: require_owner_or_root(uid))) -> UserResponse:
    try:
        user = user_service.update_user(user_id, email=update_data.email, password=update_data.password)
        return UserResponse(user_id=user['id'], email=user['email'], is_root=user['id'] == 0)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_users(current_user: AuthUser = Depends(require_root)) -> List[UserResponse]:
    ...
    return [UserResponse(user_id=u['id'], email=u['email'], is_root=u['id'] == 0) for u in users]
...
@app.post('/discord')
def send_discord_message(message: DiscordMessage, current_user: AuthUser = Depends(require_root)) -> dict[str, str]:
